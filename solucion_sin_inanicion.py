...
### Solución sin inanición
# Añadimos los límites de espera
WAIT_LIMIT_N_CARS_NORTH = 5
WAIT_LIMIT_N_CARS_SOUTH = 5
WAIT_LIMIT_NUMBER_PED = 5
...
class Monitor():
    def __init__(self):
        ...
        ### Solución sin inanición
        #Condiciones inanición
        self.n_car_north_waiting = Value('i', 0)
        self.n_car_south_waiting = Value('i', 0)
        self.n_ped_waiting  = Value('i', 0)
        
        self.wait_car_north = Condition(self.mutex)
        self.wait_car_south = Condition(self.mutex)
    ...
    def wants_enter_car(self, direction: int) -> None:
        ...
        if direction == NORTH:
            ...
            ### Solución sin inanición
            self.n_car_north_waiting.value -= 1
            if self.n_car_north_waiting.value <= WAIT_LIMIT_N_CARS_NORTH:
                self.can_car_south.notify_all()
                self.can_ped.notify_all()

        else:
            ### Solución sin inanición
            self.n_car_south_waiting.value += 1
            self.wait_car_south.wait_for(self.waiting_car_south)
            ...
            ### Solución sin inanición
            self.n_car_south_waiting.value -= 1
            if self.n_car_south_waiting.value <= WAIT_LIMIT_N_CARS_SOUTH:
                self.can_ped.notify_all()
    ...
    def wants_enter_pedestrian(self) -> None:
        ...
        #### Código añadido
        ### Solución sin inanición
        self.n_ped_waiting.value += 1
        self.wait_ped.wait_for(self.waiting_ped)
        ...
        ### Solución sin inanición
        self.n_ped_waiting.value -= 1
        if self.n_ped_waiting.value <= WAIT_LIMIT_NUMBER_PED:
            self.wait_car_north.notify_all()
            self.wait_car_south.notify_all()
    ...
    ### Solución sin inanición
    def waiting_car_north(self) -> bool:
        condition_south = self.n_car_south_waiting.value <= WAIT_LIMIT_N_CARS_SOUTH
        condition_ped = self.n_ped_waiting.value <= WAIT_LIMIT_NUMBER_PED
        return condition_south and condition_ped
    ### Solución sin inanición
    def waiting_car_south(self) -> bool:
        condition_north = self.n_car_north_waiting.value <= WAIT_LIMIT_N_CARS_NORTH
        condition_ped = self.n_ped_waiting.value <= WAIT_LIMIT_NUMBER_PED
        return condition_north and condition_ped
    ### Solución sin inanición
    def waiting_ped(self) -> bool:
        condition_north = self.n_car_north_waiting.value <= WAIT_LIMIT_N_CARS_NORTH
        condition_south = self.n_car_south_waiting.value <= WAIT_LIMIT_N_CARS_SOUTH
        return condition_north and condition_south
